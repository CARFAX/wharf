...
renderer.heading = function (text, level) {
    var cls = level == 1 ? "class='object-header'"
        : level == 2 ? "class='member-header'"
        : '',
        template = "<h{level} "+ cls + " id='{anchor}'>{text}</h{level}>",
        anchor = text;
    ...
    template = "<div class='header-container h"+(level + 1)+"'>" + template + "</div>";
